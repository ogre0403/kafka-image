...
public class SimpleExample {
    public static void main(String args[]) {
        ...
        try {
            example1.run(maxReads, topic, 0, seeds, port);
            //example2.run(maxReads, topic, 1, seeds, port);
        } catch (Exception e) {
...
//        long readOffset = getLastOffset(consumer,a_topic, a_partition, kafka.api.OffsetRequest.LatestTime(), clientName);
        ...
        while (a_maxReads > 0) {
            ...
            for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(a_topic, a_partition)) {
                ...
                numRead++;
                a_maxReads--;
                toDisk(bytes, numRead, a_partition);
            }
            //a_maxReads = 0;
            /*
            if (numRead == 0) {
    ...
    private void toDisk(byte[] ba, long num, int pa){

        try {
            BufferedImage img = ImageIO.read(new ByteArrayInputStream(ba));
            File outputfile = new File("d:/result/"+pa+"_"+num+".jpg");
